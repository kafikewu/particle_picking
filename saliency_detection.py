...
base_directory = "/shared/home/v_kafi_khan/results/emd_2.5_9_9_10000_None/"
def saliency_detection(a, gaussian_sigma, gabor_sigma, gabor_lambda, cluster_center_number,
    ...
    original_tomogram = copy.deepcopy(a)
    b_time = time.time()
    # de-noise
    ...
    if save_flag:
        ...
        plt.imsave(base_directory + 'original.png', img, cmap='gray')
    ...
    if save_flag:
        np.save(base_directory + 'label', label)
        img = (a[:, :, int(a.shape[2] / 2)]).copy()
        k = int(a.shape[2] / 2)
        draw_color = np.min(a)
        for i in range(1, img.shape[0] - 1):
            for j in range(1, img.shape[1] - 1):
                if label[i][j][k] != label[i - 1][j][k] or label[i][j][k] != label[i + 1][j][k] or label[i][j][k] != \
                    ...
                    img[i][j] = draw_color
        plt.imsave(base_directory + 'SLIC.png', img, cmap='gray')
    del distance
    gc.collect()
    ...
    res_pool = []
    label = np.load(base_directory + 'label.npy')
    ...
    if save_flag:
        np.save(base_directory + 'feature_matrix', feature_matrix)
    ...
    if save_flag:
        np.save(base_directory + 'supervoxel_saliency', supervoxel_saliency)
    ...
    if save_flag:
        img = a[:, :, int(a.shape[2] / 2)].copy()
        plt.imsave(base_directory + 'saliency_map.png', img, cmap='gray')
        np.save(base_directory + 'saliency_map', a)
        with mrcfile.new(base_directory + 'saliency_map.mrc') as mrc:
            mrc.set_data(a)
        correlate(saliency_map, original_tomogram)
...
def correlate(saliency_map, original_tomogram):
    temp_tomo = np.zeros(saliency_map.shape, dtype=np.int8)
    for i in range(saliency_map.shape[0]):
        for j in range(saliency_map.shape[1]):
            for k in range(saliency_map.shape[2]):
                if saliency_map[i][j][k] > 0:
                    temp_tomo[i][j][k] = original_tomogram[i][j][k]
                else:
                    temp_tomo[i][j][k] = 0
    with mrcfile.new(base_directory + 'correlatated.mrc') as mrc:
        mrc.set_data(temp_tomo)      
        print("correlated file saved")          
            ...
            for k in range(saliency_map.shape[2]):
                ...
                if saliency_map[i][j][k] >= 0.9 * ref_saliency_max:
                    ...
                    if (i - dif < 0 or i + dif > saliency_map.shape[0] or
                        ...
                        n += 1
                        namemrc = base_directory + "saliency_map_subtomograms" + str(n) + ".mrc"
                        namepng = base_directory + "saliency_map_subtomograms" + str(n) + ".png"
...
def gabor_feature_single_job(a, filters, fm_i, label, cluster_center_number, save_flag):
    ...
    if save_flag:
        ...
        # save fig
        plt.imsave( base_directory + 'gabor_output(%d).png' % fm_i, img, cmap='gray')
...
if __name__ == "__main__":
    # file path
    path = "/shared/home/v_kafi_khan/emd_11866.mrc"
    ...
    start_time = time.time()
    saliency_detection(a=a, gaussian_sigma=2.5, gabor_sigma=9, gabor_lambda=9, cluster_center_number=10000,
                       multiprocessing_num=0, pick_num=None, save_flag=True)
    end_time = time.time()
